...
async function get_genre(){
  var content = await axios.get("http://iwin247.kr:3080/movie/genre")
    for(var i =0; i<content.data.genre.length; i++){
      movie[0].childNodes[7+i*5].nodeValue = content.data.genre[i];
      movie[0][i].onclick = async function(){
            await axios.put("")
      }
    }
  return content.data.genre;
}
